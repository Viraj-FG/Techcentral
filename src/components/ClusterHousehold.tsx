...
const ClusterHousehold = ({ onSubmit }: ClusterHouseholdProps) => {
  ...
  const Counter = ({ 
    ...
    <div className="glass-card p-6 flex items-center justify-between">
      <div>
        <h3 className="text-lg tracking-wide text-kaeva-slate-200">{label}</h3>
        <p className="text-xs text-kaeva-slate-400">{subtitle}</p>
      </div>
      <div className="flex items-center gap-4">
        ...
        >
          <Minus className="w-4 h-4 text-kaeva-sage" />
        </motion.button>
        <span className="text-2xl font-bold w-12 text-center text-kaeva-sage">{value}</span>
        <motion.button
          whileTap={{ scale: 0.9 }}
          ...
          transition={springTransition}
        >
          <Plus className="w-4 h-4 text-kaeva-sage" />
        </motion.button>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      {dogs > 0 && (
        <motion.div
          ...
          className="text-xs text-kaeva-teal text-center tracking-wide"
        >
