...
const VoiceOnboarding = ({ onComplete }: VoiceOnboardingProps) => {
  ...
  const handleUserFinishedSpeaking = async (transcript: string) => {
    ...
    try {
      ...
      // Parse response
      const { message, extracted, isComplete } = data;
  ...
  const handleProfileUpdate = async () => {
    ...
    setConversationState((prev) => ({ ...prev, isComplete: false }));
  ...
  const handleEnterKaeva = () => {
    ...
    // Build final profile
    const profile = {
      language: "English", // Default to English for voice mode
    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-center">
        ...
        {showSubtitles && !showSummary && (
          <VoiceSubtitles
            userText={apertureState === "thinking" ? userTranscript : ""}
            aiText={apertureState === "speaking" ? aiTranscript : ""}
          />
        )}
