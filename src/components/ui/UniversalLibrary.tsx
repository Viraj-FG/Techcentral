...
interface UniversalLibraryProps {
  ...
  standalone?: boolean; // If true, render as Sheet. If false, render as div for embedding
}
...
export const UniversalLibrary = ({
        ...
        <div className="space-y-2">
          {filteredItems.length === 0 ? (
            ...
            filteredItems.map((item: any, index: number) => {
              const displayName = item.name || item.title || item.item_name || item.food_name || 'Unknown';
  ...
  // If standalone, wrap in Sheet. Otherwise, return content directly for embedding
  if (standalone) {
    return (
