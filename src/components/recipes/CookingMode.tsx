...
import { PostCookingSheet } from './PostCookingSheet';
...
export const CookingMode = ({ recipe, onComplete, onBack }: Props) => {
  ...
  const handleActualComplete = async () => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background">
        ...
        <div className="flex items-center justify-between max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            <Button
              ...
              className="gap-2"
            >
              {viewMode === 'list' ? <Eye className="w-4 h-4" /> : <List className="w-4 h-4" />}
              {viewMode === 'list' ? 'Single Step' : 'All Steps'}
            </Button>
          ...
          {viewMode === 'single' ? (
            ...
            <div className="space-y-3 mb-12">
              {instructions.map((inst, idx) => {
                const instruction = inst?.instruction || inst?.text || inst || '';
                return (
                  <CookingStepCard
                    key={idx}
                    stepNumber={idx + 1}
      ...
      {/* Post-Cooking Sheet */}
      <PostCookingSheet
        open={showPostCooking}
        onOpenChange={(open) => {
          if (!open) {
            handleActualComplete();
          }
          setShowPostCooking(open);
        }}
